#!/bin/bash
# Submit the 12-task Slurm array for the norm-aware Qwen-2.5-7B steering sweep.
#
# Usage:
#   bash script_cos/run_steering_norm_qwen_array.sh
#   LAYERS="19" FAMILIES="medhallu" bash ...
#   DRY_RUN=1 bash ...
#
# Requires H_L_NORMS_JSON env var pointing at the Qwen h_l_norms.json.
set -euo pipefail

: "${H_L_NORMS_JSON:?H_L_NORMS_JSON not set (path to qwen h_l_norms.json)}"
...
BASELINE_CACHE_DIR="${BASELINE_CACHE_DIR:-${OUTPUT_ROOT}/steering_norm_qwen_baseline_cache_${TS}}"
LOG_DIR="${LOG_DIR:-${OUTPUT_ROOT}/steering_norm_qwen_logs_${TS}}"
...
for FAMILY in ${FAMILIES}; do
  for LAYER in ${LAYERS}; do
    jobname="steer_norm_qwen_l${LAYER}_${FAMILY}_${TS}"
    ...
    cmd=(sbatch
        ...
        script_cos/run_steering_norm_qwen_one.sbatch
    )
...
echo "Logs:     ${LOG_DIR}/"
echo "Outputs:  ${OUTPUT_ROOT}/steering_norm_qwen_l{LAYER}_{FAMILY}_${TS}/"
